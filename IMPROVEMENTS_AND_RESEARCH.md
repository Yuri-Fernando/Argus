...
| Contexto financeiro/fiscal/contábil/regulatório | ✅ implementado (extensão pedida explicitamente pelo usuário — ver [ADR-015](docs/decisions/ADR-015-fiscal-tax-reform-extension.md)) | `data/synthetic/generators/fiscal.py` + `agents/fiscal/` + `data_quality/expectations/fiscal_document.py` — dataset 100% sintético e auto-contido (fora de `lakehouse/run_pipeline.py::TABLE_ORDER`), não um dataset fiscal "de brinquedo" solto: fecha o mesmo gap de VLM (linha abaixo) ao mesmo tempo |
...
| **VLM** | ✅ implementado (ver [ADR-015](docs/decisions/ADR-015-fiscal-tax-reform-extension.md)) | `rag/local_stack/document_parser.py::DoclingVlmDocumentParser` — `VlmPipeline` do Docling (IBM Granite-Docling-258M) contra os documentos fiscais escaneados sintéticos. Testado ao vivo: roda de ponta a ponta sem erro, mas produziu saída de baixa qualidade (loop de repetição) no teste real — achado honesto documentado no ADR-015, não escondido; o OCR tradicional (RapidOCR) do pipeline padrão do Docling leu o mesmo documento melhor |
