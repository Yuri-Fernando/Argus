...
**AWS Bedrock** (`_complete_bedrock()`, added Sprint 17 — ROADMAP.md, per the AWS-Bedrock gap in [IMPROVEMENTS_AND_RESEARCH.md §5](../../IMPROVEMENTS_AND_RESEARCH.md#5-integração-dos-requisitos-de-tributariotxt--complemento-de-add2txt-agosto2026)) gets its own adapter too — Bedrock's `invoke_model()` takes a model-family-specific JSON body (Anthropic-on-Bedrock's shape differs from Bedrock's other model families), so it cannot reuse the OpenAI-compatible mapping. It follows the exact same documented-stub pattern as every other not-yet-credentialed adapter: a docstring with the real `boto3.client("bedrock-runtime")`/`invoke_model()` wiring, a stub body returning `""` until `AWS_ACCESS_KEY_ID`/`AWS_SECRET_ACCESS_KEY`/`AWS_REGION` (`.env.example`) are real. `boto3` is intentionally not added to `pyproject.toml`'s core `dependencies` — this project is Azure-primary (ADR-001); Bedrock is a documented alternative, not a default, so no `defaults` entry in `models.yaml` routes to it. Use it explicitly: `complete(prompt, "final_reasoning", model_id="bedrock-claude-3-5-sonnet")`.
