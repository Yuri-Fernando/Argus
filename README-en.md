...
flowchart TD
    ...
    CORTEX --> MCP
    DBX -. Genie MCP .-> MCP
    MCP --> AGENTS[Agentic AI\nCustomer · Data Quality · Recommendation · Monitoring · Knowledge Ingestion · Fiscal]
...
`Python` `PySpark` `Azure Data Factory` `Azure Databricks` `Delta Lake` `Unity Catalog` `MLflow` `dbt / MetricFlow` `Great Expectations (GX Core)` `Snowflake` `Snowflake Cortex (Analyst/Search/Agents/AI Functions)` `Power BI` `LangGraph` `Agno` `Azure OpenAI` `LLM Gateway (OpenAI/Gemini/DeepSeek/Bedrock)` `MCP (Model Context Protocol)` `A2A (Agent2Agent)` `Crawl4AI` `Docling (text + VLM)` `ChromaDB` `FAISS` `FastAPI` `Terraform` `Kubernetes` `Docker` `GitHub Actions` `OpenTelemetry` `Prometheus` `Grafana` `NetworkX` — full rationale per tool in [ARCHITECTURE.md §3](ARCHITECTURE.md#3-tech-stack-rationale).

### Fiscal / tax-reform extension (ADR-015)

A dedicated module simulates Brazilian tax-reform invoice line items (IBS/CBS/Imposto Seletivo) — fully synthetic, with deliberately injected discrepancies, its own root-cause diagnosis agent, and the real use case that closes the platform's VLM pipeline: synthetically generated *scanned* fiscal documents processed end-to-end. Full write-up, including an honest engineering finding (a small general-purpose VLM underperforming traditional OCR on a plain-text document), in [`docs/decisions/ADR-015-fiscal-tax-reform-extension.md`](docs/decisions/ADR-015-fiscal-tax-reform-extension.md).
...
| Power BI MCP | **Experimental** | Still Public Preview as of mid-2026 |
| Databricks Managed MCP (Genie) | Production-like | Reached GA in early 2026 |
| Terraform (Azure/Databricks/Snowflake) | Production-like | 20/20 modules validated (`terraform validate`), `apply` not yet run against real accounts |
| Kubernetes manifests | Production-like | Declarative, `kubectl apply --dry-run=client` validated |
| Agents (LangGraph, Agno, custom MCP, A2A) | Prototype → Production-like | Human-in-the-loop required for all write actions |
| VLM pipeline (Docling + Granite-Docling-258M) | **Experimental** | Runs end-to-end for real, but produced low-quality output on the one document tested live — see ADR-015 |
...
- **Synthetic CRM / Marketing / Support / Web Events / Finance / Fiscal** — generated with reproducible seeds, deliberately containing duplicates, missing fields and inconsistencies (including tax-reform-specific ones: invalid NCM codes, incompatible CST/CFOP combinations, out-of-range tax rates) to give the MDM/DQ pipeline something real to solve.
- **Synthetic corporate documents** (refund/delivery/loyalty/privacy/tax-reform policies) — feed RAG / Cortex Search; a subset of the fiscal documents is also rendered as scanned images for the VLM pipeline.
...
| [docs/decisions/](docs/decisions/) | ADRs (PT-BR) — 15 architecture decisions documented |
