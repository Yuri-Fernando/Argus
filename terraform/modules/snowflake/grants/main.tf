...
resource "snowflake_grant_privileges_to_account_role" "transformer_analytics" {
  account_role_name = var.role_names["TRANSFORMER"]
...
resource "snowflake_grant_privileges_to_account_role" "bi_reader_semantic" {
  account_role_name = var.role_names["BI_READER"]
  privileges        = ["USAGE"]
...
resource "snowflake_grant_privileges_to_account_role" "ai_agent_semantic_and_ai" {
  for_each = toset([var.schema_names["SEMANTIC"], var.schema_names["AI"]])

  account_role_name = var.role_names["AI_AGENT"]
